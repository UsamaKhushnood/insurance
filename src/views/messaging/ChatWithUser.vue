  ...
  <div class="chat">
    <div class="chat-chat-box">
      ...
      <div class="chatbox-body bg-white">
        ...
        <div
          class="messages-box"
          v-for="(chat, index) in getReceiverMsg"
          :key="index"
        >
          <div
            class="my-messages"
            v-if="chat.sender === getReceiver.firebase_uid"
          >
            <div class="my-avatar">
              <b-avatar
                variant="info"
          ...
          <div
            class="user-messages"
            v-if="chat.sender === getUser.agent.firebase_uid"
          >
            <div class="user-avatar">
              <b-avatar
                variant="info"
  ...
  methods: {
    ...
    setConversion() {
      const rec_msg2 = db
        .database()
        .ref("/Conversation/")
        .child(this.getUser.agent.firebase_uid)
        .child(this.getReceiver.firebase_uid)
        .set({
    ...
    sendMessage() {
      ...
      var container = this.$el.querySelector(
        ".chat .chat-chat-box .chatbox-body"
      );
      ...
      setTimeout(function () {
        ...
        allConversion.filter((item) => {
          ...
          if (
            ...
            (item.sender === user && item.reciever === user) ||
            item.sender === reciever
          ) {
            mtArr.push(item);
          }
        });
